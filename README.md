...
**وضعیت:** جریان کامل خرید فاز ۱ پیاده شد — کاتالوگ (وزنی/عددی با مقدار اعشاری) ← سبد ← checkout مهمان ← پرداخت (Zarinpal + درگاه mock) ← تأیید سفارش، به‌علاوه کد تخفیف و ورود اختیاری با OTP. (MVP، ~۱۰ سفارش/هفته، بدون Redis/Postgres/Celery). باقی‌مانده: SEO (sitemap/JSON-LD)، جستجو، محاسبه دقیق پست، فاکتور PDF، سخت‌سازی امنیتی نهایی.
...
- **فاز ۱ (در جریان):**
  ...
  - ⏳ باقی‌مانده: SEO (sitemap/JSON-LD)، جستجو، محاسبه دقیق پست پیشتاز، فاکتور PDF، Sentry، تست E2E، gate کردن `/_styleguide/`
- **فاز ۲:** Postgres، Redis، Celery، CDN، بین‌الملل (NOWPayments)

اپ‌ها: `catalog`، `cart`، `orders`، `payments`، `coupons`، `accounts`. تست‌ها: `pytest` (۸۹ تست). داده نمونه: `python manage.py seed_catalog`.
