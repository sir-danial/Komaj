...
def _add_to_cart(client, variant, qty="2"):
...
def test_checkout_page_renders(client, box_variant):
    _add_to_cart(client, box_variant)
    resp = client.get("/checkout/")
    assert resp.status_code == 200
    assert "تسویه حساب" in resp.content.decode()


def test_checkout_creates_order_and_redirects_to_payment(client, box_variant, valid_checkout_data):
    _add_to_cart(client, box_variant, "2")
    resp = client.post("/checkout/", valid_checkout_data)
    assert resp.status_code == 302
    order = Order.objects.get()
    assert resp.url == f"/payment/start/{order.token}/"
    # totals: 2 * 180000 = 360000; vat 32400; post 45000 -> 437400
    assert order.subtotal == Decimal("360000")
    assert order.vat_amount == Decimal("32400")
    assert order.shipping_cost == Decimal("45000")
    assert order.total == Decimal("437400")
...
def test_checkout_snapshots_line_item(client, box_variant, valid_checkout_data):
    _add_to_cart(client, box_variant, "1")
    client.post("/checkout/", valid_checkout_data)
    item = Order.objects.get().items.get()
    assert item.product_name == "کلمپه"
    assert item.variant_label == "جعبه یک کیلویی"
    assert item.unit_label == "جعبه"
    assert item.quantity == 1
    assert item.unit_price == Decimal("180000")
    assert item.line_total == Decimal("180000")


def test_checkout_invalid_phone_rerenders(client, box_variant, valid_checkout_data):
    _add_to_cart(client, box_variant)
...
def test_tehran_courier_rejected_for_other_province(client, box_variant, valid_checkout_data):
    _add_to_cart(client, box_variant)
...
def test_terms_required(client, box_variant, valid_checkout_data):
    _add_to_cart(client, box_variant)
...
def test_confirmation_page_by_token(client, box_variant, valid_checkout_data):
    _add_to_cart(client, box_variant)
    client.post("/checkout/", valid_checkout_data)
    order = Order.objects.get()
...
def test_stale_cart_rejected(client, box_variant, valid_checkout_data):
    _add_to_cart(client, box_variant, "2")
    # stock drops below ordered qty after item was in cart
    box_variant.stock_qty = 1
    box_variant.save()
    resp = client.post("/checkout/", valid_checkout_data)
    assert resp.status_code == 302
    assert resp.url == "/cart/"
